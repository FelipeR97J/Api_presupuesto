...
router.get('/', async (req, res) => {
  try {
    ...
    const total = await ExpenseCategory.findAll({
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!category) {
      return res.status(404).json(formatError(ErrorCodes.EXPENSE_CATEGORY.CATEGORY_NOT_FOUND));
    }
...
router.post('/', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (!name) {
      return res.status(400).json(formatError(ErrorCodes.EXPENSE_CATEGORY.NAME_REQUIRED));
    }
    ...
    if (existingCategory) {
      return res.status(400).json(formatError(ErrorCodes.EXPENSE_CATEGORY.DUPLICATE_NAME));
    }
 ...
 * - No se puede inactivar si tiene gastos activos asociados
 * REQUIERE AUTENTICACIÓN (token JWT)
 */
router.patch('/:id', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    if (category.get('isSystem') === true) {
      return res.status(403).json(formatError(ErrorCodes.EXPENSE_CATEGORY.SYSTEM_CATEGORY_CANNOT_MODIFY));
    }

    // Validar: Solo el usuario que creó puede actualizar
    if (category.get('createdBy') !== req.user.id) {
      return res.status(403).json(formatError(ErrorCodes.EXPENSE_CATEGORY.PERMISSION_DENIED));
    }

    // Si se intenta inactivar, verificar que no tenga gastos activos
    if (id_estado === 2 && (category as any).id_estado === 1) {
      ...
      if (expenseCount > 0) {
        return res.status(400).json(formatError(ErrorCodes.EXPENSE_CATEGORY.CANNOT_DEACTIVATE_WITH_EXPENSES));
      }
    }
 ...
 * - No se puede si tiene gastos activos asociados
 * REQUIERE AUTENTICACIÓN (token JWT)
 */
router.delete('/:id', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    // Verificar si la categoría tiene gastos activos asociados
    const expenseCount = await Expense.count({
      where: { categoryId: req.params.id, id_estado: 1 }, // 1 = Activo
    });

    if (expenseCount > 0) {
      return res.status(400).json(formatError(ErrorCodes.EXPENSE_CATEGORY.CANNOT_DELETE_WITH_EXPENSES));
    }
